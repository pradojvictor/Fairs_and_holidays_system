    ...
    <div className="dashboard-layout">
      ...
      <header className="dashboard-header">
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={() => setIsEventModalOpen(true)} style={{ padding: '0.5rem 1rem', backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>+ Nova Ausência</button>
          <button className="btn-header-logout" onClick={handleLogout}>Sair</button>
        </div>
      </header>

      <EventModal isOpen={isEventModalOpen} onClose={() => setIsEventModalOpen(false)} professionals={dbData.professionals} onDataUpdated={loadData} />
      
      <EventDetailModal 
        isOpen={!!selectedEvent} event={selectedEvent} 
        professional={dbData.professionals.find(p => p.id === selectedEvent?.professionalId)} 
        onClose={() => setSelectedEvent(null)} onDelete={handleDeleteEvent} 
      />
